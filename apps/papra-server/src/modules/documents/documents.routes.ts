...
function setupUpdateDocumentRoute({ app, db, eventServices }: RouteDefinitionContext) {
  app.patch(
    ...
    }).refine(data => data.name !== undefined || data.content !== undefined || data.documentDate !== undefined || data.folderId !== undefined, {
      message: 'At least one of \'name\', \'content\', \'documentDate\', or \'folderId\' must be provided',
    })),
    async (context) => {
      const { userId } = getUser({ context });
      const { organizationId, documentId } = context.req.valid('param');
